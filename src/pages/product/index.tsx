            ...
            <div className="flex flex-col w-full h-full py-4">
                ...
                <div className="h-full w-full rounded-3xl p-4 overflow-y-auto">
                    <div className="py-4 px-4 text-right flex justify-between">
                        <h1 className="text-3xl font-semibold text-slate-600">Add New Categories and Products</h1>
                        <PrimaryLink href={'/category/create'} text="Add Category" />
                    </div>
                    <div className="bg-white shadow-xl rounded-3xl w-full h-full px-8 overflow-y-auto">
                        {!search.length ? <>{
                            categoriesWithProducts.isLoading ? <Spinner /> : <div>
                                {categoriesWithProducts.data?.categoriesWithProducts.map((category) => {
                                    return (category.products.length != 0) && (<section className="pt-4" id={category.name} key={category.id}>
